...
| `vision_analyze` | 识别本地图片（6 模式：general / ocr / table / code / error / **evidence 结构化证据** + `budget` 含 `mega` 16M 像素） |
| `vision_ocr` | 独立 OCR，保留原始排版 |
| `vision_ground` | 目标定位（如「微信图标」），返回像素 + 归一化坐标 |
| `vision_detect` | 枚举一类元素，编号 + 像素坐标框 |
| `vision_dominant_colors` | 主色分析（本地像素算法，无需视觉 API） |
| `vision_long_screenshot_ocr` | 超长截图分块 OCR：本地 tesseract 优先 → VLM 兜底 → 合并 |
| `vision_clipboard` | 剪贴板图片兜底识别（paste-to-path 失败/特殊场景的手动通道） |
| `vision_activate` | 渐进式暴露兜底：skill 加载后工具未自动出现时调用一次 |

## 工程化要点

- **渐进式工具暴露**：全局只挂 1 个轻量激活工具，完整工具集在 skill 加载后按 Agent 挂载（省上下文）；`progressive: false` 可回退为全局注册
- **paste-to-path 直贴（v0.4.2 起输入框显示 📎 chip）**：client 在 capture 阶段截获粘贴图片，发送时同源 POST 落盘到 `.dsh-vision/pasted/`，消息里**没有** image 块 → DSH 不再报 `MODEL_DOES_NOT_SUPPORT_IMAGES`；旧版无 reference 能力时自动回退为插入路径文本
- **多 provider failover + 429 退避**：`visionProviders` 数组按顺序 failover；每个 provider 配 `apiUrl/model/apiKey/credential`；429 按 `Retry-After` 退避重试一次
...
- **本地 OCR 快路径**：长截图每块先跑 tesseract（不可用才调 VLM，省 token 更快）
- **图像记忆缓存**：按图片 SHA-256 + mode/budget/crop/prompt 缓存，TTL 内命中直接返回 `cached:true`
...
Qwen 官方动态分辨率预处理（`smart_resize`：预算像素 + patch 网格吸附）→ 任意 OpenAI 兼容多模态模型（默认 MiniMax-M3，`thinking: disabled` 关思考，可替换）。Grounding 用 Qwen 官方方法：VLM 输出 0-1000 归一化 bbox → 解析 JSON / `<ref><box>` 双格式 → 映射像素坐标。

## 图片投递（三种）

| 方式 | 操作 | 适用 |
|---|---|---|
| ① 路径直发 | 对话框发路径文本："识别这张图 `E:\...\xxx.png`" | 所有环境 |
| ② 剪贴板 | Win+Shift+S 截屏 → "看图" → `vision_clipboard` 自动保存到工作区 `.dsh-vision/` | 所有环境 |
| ③ 直接贴图 | v0.4 起内置：粘贴 → 同源上传 `.dsh-vision/pasted/` → 路径文本进入消息 → 模型调 `vision_analyze` | ✅ 所有环境，**不需 pi-ai 补丁** |

pi-ai 适配器（opencode-go 等）的 image→path 旧补丁仅作兼容保留；dsh 升级后若旧补丁被重新打上，可运行 `scripts/restore_pi_ai_vision_patch.py` 还原。
...
安装后插件包自带的 `cordis.patch.yml` 会自动贡献 `id: vision-skill` 条目，**无需手动 insert**。配置默认值由插件内置 Schemastery Config schema 提供（`apiUrl`=MiniMax / `model`=MiniMax-M3 / `credential`=VISION_API_KEY）。

### 配置（覆盖 bundle 默认值）

⚠️ **不要**在 profile 的 `cordis.patch.yml` 里再 `insert` 一个 `id: vision-skill`——重复 id 会导致 `duplicate loader entry id` 启动崩溃。需要自定义配置时用**裸条目按 id 覆盖**（不带 `insert:` 包装）：
...
- 同名技能冲突：本插件以 `runtime` 层注册技能名 `vision`；若同时在 `$DSH_HOME/skills`（user 层）或项目 `.dsh/skills`（project 层）安装同名技能，按官方优先级 project > runtime > user 可能互相遮蔽——建议二选一安装
