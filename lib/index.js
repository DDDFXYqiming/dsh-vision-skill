...
export const Config = Schema.object({
	...
	// vision_clipboard 保存图片时 PowerShell 子进程超时。
	clipboardTimeoutMs: Schema.number().min(5000).max(600000).default(60000),
...
/**
 * 默认值唯一来源是 Config schema（调用式校验，schema 本身即 Standard Schema）。
 * Cordis 加载时已校验填充过一次；此处对已填充配置为幂等操作，
 * 对测试/脚本直调的部分配置则补齐默认值。
 */
...
async function resolveApiKey(ctx, cfg) {
	...
	throw new Error(
		...
		`ctx.get=${typeof viaGet !== "undefined"}, credentials 服务=${creds ? "存在" : "缺失"}）。请在 config.apiKey、credentials 或 provider 配置中提供密钥。`
	);
}
...
async function saveClipboardImage(cfg, dest, signal) {
	...
	const { stderr } = await run(cfg.pwsh, ["-NoProfile", "-NonInteractive", "-Command", script], {
		env: process.env,
		timeoutMs: cfg.clipboardTimeoutMs,
		signal
	});
